...
import com.infinixsoft.TexturePacker.LentoRapido;
...
public class LentoRapidoScene {
	...
	private static String backgroundPath = "gfx/bglentorapido.png";
	private static String texturePackerName = "lentorapido.xml";
	...
	public static void load(GameActivity myActivity) {
		...
		//getinfobutton
		scene = context.getInfoButtonSprite(scene, SceneType.LENTORAPIDO);
		
		LentoRapido texturePackerInterface = new LentoRapido() {
		};
		...
		// Add items to scene
		addItem(item1TextureRegion, 130, 114, 1);
		addItem(item2TextureRegion, 283, 146, 2);
		addItem(item3TextureRegion, 432, 118, 1);
		addItem(item5TextureRegion, 686, 127, 1);
		addItem(item4TextureRegion, 523, 134, 2);
		addItem(item6TextureRegion, 787, 146, 2);
		...
		Sprite type1Sprite = new Sprite(42, 451, type1TextureRegion, context.getVertexBufferObjectManager());
		scene.attachChild(type1Sprite);

		Sprite type2Sprite = new Sprite(787, 496, type2TextureRegion, context.getVertexBufferObjectManager());
		...
		final SpriteType sprite = new SpriteType(pX, pY, textureRegion, context.getVertexBufferObjectManager(), type) {
			...
			private void putInBox(final SpriteType sprite, int box) {
				...
				if (box == 1) {
					...
					float rotation = new Random().nextInt(40) - 20;
					sprite.registerEntityModifier(new RotationModifier(0.3f, sprite.getRotation(), rotation) {
						protected void onModifierFinished(org.andengine.entity.IEntity pItem) {
...
//					CutScene.load(context);
					...
					sprite.registerEntityModifier(new DelayModifier(1f) {
						protected void onModifierFinished(org.andengine.entity.IEntity pItem) {
							SceneManager.setScene(CutScene.run(SceneType.LENTORAPIDO));
						};
					});
				}
			}
