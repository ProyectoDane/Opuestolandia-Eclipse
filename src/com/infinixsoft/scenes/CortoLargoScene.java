...
import com.infinixsoft.TexturePacker.CortoLargo;
...
public class CortoLargoScene {
	...
	private static String backgroundPath = "gfx/bgcortolargo.png";
	private static String texturePackerName = "cortolargo.xml";
	...
	public static void load(GameActivity myActivity) {
		...
		//getinfobutton
		scene = context.getInfoButtonSprite(scene, SceneType.CORTOLARGO);

		CortoLargo texturePackerInterface = new CortoLargo() {
		};
		...
		// Add items to scene
		addItem(item1TextureRegion, 149, 139, 1);
		addItem(item2TextureRegion, 252, 131, 2);
		addItem(item4TextureRegion, 463, 129, 2);
		addItem(item3TextureRegion, 407, 146, 1);
		addItem(item5TextureRegion, 708, 135, 1);
		addItem(item6TextureRegion, 808, 147, 1);
		...
		Sprite type1Sprite = new Sprite(146, 512, type1TextureRegion, context.getVertexBufferObjectManager());
		scene.attachChild(type1Sprite);

		Sprite type2Sprite = new Sprite(808, 404, type2TextureRegion, context.getVertexBufferObjectManager());
		...
		final SpriteType sprite = new SpriteType(pX, pY, textureRegion, context.getVertexBufferObjectManager(), type) {
			...
			private void putInBox(final SpriteType sprite, int box) {
				...
				if (box == 1) {
					...
					float rotation = 80 + new Random().nextInt(20);
					sprite.registerEntityModifier(new RotationModifier(0.3f, sprite.getRotation(), rotation) {
						protected void onModifierFinished(org.andengine.entity.IEntity pItem) {

							// int max = (int) (195 - (sprite.getHeight()));
							int max = (int) (160 - (sprite.getHeight()));
							Random rnd = new Random();
							int posX = rnd.nextInt(max);
							// int posX = rnd.nextInt(100)+45;
							int posY = rnd.nextInt((int) (sprite.getWidth() / 2));
				...
				if (placedItems == totalItems) {
					...
					sprite.registerEntityModifier(new DelayModifier(1f) {
						protected void onModifierFinished(org.andengine.entity.IEntity pItem) {
							SceneManager.setScene(CutScene.run(SceneType.CORTOLARGO));
						};
					});
				}
			}
