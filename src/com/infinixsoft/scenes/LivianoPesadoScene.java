...
import com.infinixsoft.TexturePacker.LivianoPesado;
...
public class LivianoPesadoScene {
	...
	private static int placedItems = 0, totalItems = 5;
	private static String backgroundPath = "gfx/bglivianopesado.png";
	private static String texturePackerName = "pesadoliviano.xml";
	...
	public static void load(GameActivity myActivity) {
		...
		//getinfobutton
		scene = context.getInfoButtonSprite(scene, SceneType.LIVIANOPESADO);

		LivianoPesado texturePackerInterface = new LivianoPesado() {
		};
		...
		// Add items to scene
		addItem(item1TextureRegion, 142, 114, 1);
		addItem(item2TextureRegion, 256, 56, 2);
		addItem(item3TextureRegion, 459, 112, 1);
		addItem(item4TextureRegion, 575, 60, 2);
		addItem(item5TextureRegion, 790, 118, 1);
		...
		Sprite type1Sprite = new Sprite(116, 488, type1TextureRegion, context.getVertexBufferObjectManager());
		scene.attachChild(type1Sprite);

		Sprite type2Sprite = new Sprite(802, 446, type2TextureRegion, context.getVertexBufferObjectManager());
				...
				if (box == 1) {
					...
					sprite.registerEntityModifier(new RotationModifier(0.3f, sprite.getRotation(), rotation) {
						protected void onModifierFinished(org.andengine.entity.IEntity pItem) {
							...
							int max = (int) ((195 - (sprite.getHeight()))-20);
							Random rnd = new Random();
							int posX = rnd.nextInt(max) + 45;
...
//					CutScene.load(context);
					...
					sprite.registerEntityModifier(new DelayModifier(1f) {
						protected void onModifierFinished(org.andengine.entity.IEntity pItem) {
							SceneManager.setScene(CutScene.run(SceneType.LIVIANOPESADO));
						};
					});
				}
			}
