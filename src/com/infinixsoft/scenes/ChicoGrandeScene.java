...
import com.infinixsoft.TexturePacker.ChicoGrande;
...
public class ChicoGrandeScene {
	...
	private static String backgroundPath = "gfx/bgchicogrande.png";
	private static String texturePackerName = "chicogrande.xml";
	...
	public static void load(GameActivity myActivity) {
		...
		//getinfobutton
		scene = context.getInfoButtonSprite(scene, SceneType.CHICOGRANDE);

		ChicoGrande texturePackerInterface = new ChicoGrande() {
		};
		...
		// Add items to scene
		addItem(item1TextureRegion, 145, 50, 2);
		addItem(item2TextureRegion, 296, 144, 1);
		addItem(item3TextureRegion, 378, 34, 2);
		addItem(item4TextureRegion, 558, 137, 1);
		addItem(item5TextureRegion, 647, 40, 2);
		addItem(item6TextureRegion, 830, 133, 1);
		...
		Sprite type1Sprite = new Sprite(148, 524, type1TextureRegion, context.getVertexBufferObjectManager());
		scene.attachChild(type1Sprite);

		Sprite type2Sprite = new Sprite(836, 443, type2TextureRegion, context.getVertexBufferObjectManager());
...
//					CutScene.load(context);
					...
					sprite.registerEntityModifier(new DelayModifier(1f) {
						protected void onModifierFinished(org.andengine.entity.IEntity pItem) {
							SceneManager.setScene(CutScene.run(SceneType.CHICOGRANDE));
						};
					});
				}
			}
