...
import com.infinixsoft.TexturePacker.PocoMucho;
...
public class PocoMuchoScene {
	...
	private static String backgroundPath = "gfx/bgpocomucho.png";
	private static String texturePackerName = "pocomucho.xml";
	...
	public static void load(GameActivity myActivity) {
		...
		//getinfobutton
		scene = context.getInfoButtonSprite(scene, SceneType.POCOMUCHO);

		PocoMucho texturePackerInterface = new PocoMucho() {
		};
		...
		// Add items to scene
		addItem(item1TextureRegion, 139, 42, 2);
		addItem(item3TextureRegion, 407, 10, 2);
		addItem(item2TextureRegion, 305, 91, 1);
		addItem(item4TextureRegion, 611, 62, 1);
		addItem(item5TextureRegion, 761, 89, 1);
		...
		Sprite type1Sprite = new Sprite(118, 460, type1TextureRegion, context.getVertexBufferObjectManager());
		scene.attachChild(type1Sprite);

		Sprite type2Sprite = new Sprite(805, 409, type2TextureRegion, context.getVertexBufferObjectManager());
				...
				if (box == 1) {
					...
					sprite.registerEntityModifier(new RotationModifier(0.3f, sprite.getRotation(), rotation) {
						protected void onModifierFinished(org.andengine.entity.IEntity pItem) {
							...
							int max = (int) (197 - (sprite.getWidth()));
							Random rnd = new Random();
							int posX = rnd.nextInt(max) + 45;
							int posY = rnd.nextInt((int) (sprite.getHeight() / 2)-10);
...
//					context.toastOnUIThread("ok!");
					...
					sprite.registerEntityModifier(new DelayModifier(1f) {
						protected void onModifierFinished(org.andengine.entity.IEntity pItem) {
							SceneManager.setScene(CutScene.run(SceneType.POCOMUCHO));
						};
					});
				}
			}
