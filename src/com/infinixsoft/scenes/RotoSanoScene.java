...
import com.infinixsoft.TexturePacker.RotoSano;
...
public class RotoSanoScene {
	...
	private static int placedItems = 0, totalItems = 7;
	private static String backgroundPath = "gfx/bgrotosano.png";
	private static String texturePackerName = "rotosano.xml";
	...
	public static void load(GameActivity myActivity) {
		...
		//getinfobutton
		scene = context.getInfoButtonSprite(scene, SceneType.ROTOSANO);
		
		RotoSano texturePackerInterface = new RotoSano() {
		};
		...
		// Add items to scene
		addItem(item1TextureRegion, 130, 100, 1);
		addItem(item2TextureRegion, 221, 79, 2);
		addItem(item3TextureRegion, 343, 57, 1);
		addItem(item4TextureRegion, 479, 101, 2);
		addItem(item6TextureRegion, 683, 111, 1);
		addItem(item5TextureRegion, 591, 81, 1);
		addItem(item7TextureRegion, 769, 59, 2);
		...
		Sprite type1Sprite = new Sprite(88, 449, type1TextureRegion, context.getVertexBufferObjectManager());
		scene.attachChild(type1Sprite);

		Sprite type2Sprite = new Sprite(820, 450, type2TextureRegion, context.getVertexBufferObjectManager());
